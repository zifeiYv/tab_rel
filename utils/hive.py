            ...
            if i == processes - 1:
                p = multiprocessing.Process(target=pre_processing, name=f'preprocess-Process-{i}',
                                            args=(model_id, tables[i * batch_size:],
                                                  True, host, port, user, password, database, q,))
            else:
                p = multiprocessing.Process(target=pre_processing, name=f'preprocess-Process-{i}',
            ...
            if i == processes - 1:
                p = multiprocessing.Process(target=find_rel, name=f'rel-Process-{i}',
                                            args=(rel_cols_items[i * batch_size:], pks,
                                                  model_id, True, host, port, user, password, database, q,))
            else:
                p = multiprocessing.Process(target=find_rel, name=f'rel-Process-{i}',
...
def pre_processing(model_id, tables, multi, host, port, user, password, database, q=None):
...
def find_rel(rel_cols, pks, model_id, multi, host, port, user, password, database, q=None):
    """查找关系。
            ...
            for pk_tab in pks:
                ...
                for pk in pks[pk_tab]:
                    ...
                    if flag:
                        ...
                        res = [model_id, database, pk_tab, 'table1comment', pk, 'column1comment',
                               model_id, database,
